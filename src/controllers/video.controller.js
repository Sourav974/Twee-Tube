...
const publishAVideo = asyncHandler(async (req, res) => {
  ...
  if (!videoFile) throw new ApiError(400, "Video file is not found");

  if (!thumbnail) throw new ApiError(400, "Thumbnail is not found");
...
const updateVideo = asyncHandler(async (req, res) => {
  ...
  if (!updateVideo)
    throw new ApiError("Failed to update video please try again!!");

  if (updateVideo) await deleteOnCloudinary(thumbnailToDelete);

  res
    .status(200)
    .json(new ApiResponse(200, updateVideo, "Video updated succesffuly"));
});
...
const deleteVideo = asyncHandler(async (req, res) => {
  ...
  if (video.owner.toString() !== req.user._id.toString())
    throw new ApiError(400, "You can't delte this video you are not the owner");
  ...
  if (!videoDeleted)
    throw new ApiError(
      400,
      "Failed to delete this video please try again later!"
    );
  ...
  res.status(201).json(new ApiResponse(201, {}, "Video deleted successfully"));
});
...
const togglePublishStatus = asyncHandler(async (req, res) => {
  ...
  if (video.owner.toString() !== req.user._id.toString)
    throw new ApiError(400, "You are not auhtorized to toggle publish status");
